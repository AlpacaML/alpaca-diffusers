...
It is important to specify `output_type="latent"` in the pipeline to keep all the outputs in _latent_ space to avoid an unnecessary decode-encode step. This only works if the chained pipelines are using the same VAE.
